...
GenerateMipsPSO::GenerateMipsPSO()
{
    ...
    defaultUAV = Application::AllocateDescriptors(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, 4);

    // Allocate one CPU descriptor for each mip level generated per compute (4)
    for (UINT i = 0; i < 4; ++i)
    {
