...
{
public:
    // Used internally to manage allocations
    DescriptorAllocatorPage(D3D12_DESCRIPTOR_HEAP_TYPE _type, uint32_t _numDescriptors);
    ...
    // Allocate a number of descriptors from this descriptor heap. If the allocation cannot be satisfied, then a NULL descriptor is returned
    DescriptorAllocation Allocate(uint32_t numDescriptors);

    // Return a descriptor back to the heap.
    // Stale descriptors are not freed directly, but put on a stale allocations queue. Stale allocations are returned to the heap using the DescriptorAllocatorPage::ReleaseStaleAllocations method
    // DescriptorAllocation&& implies that the original DescriptorAllocation is moved into the function and the original is invalid
...
protected:
    ...
    // Used to determine where a descriptor needs to be placed back in heap when the descriptor is freed
...
private:
    ...
    {
        StaleDescriptorInfo(OffsetType offset, SizeType size, uint64_t frame) : Offset(offset), Size(size), FrameNumber(frame) {}
