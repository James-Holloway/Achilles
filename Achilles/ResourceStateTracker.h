...
public:
	...
	// resource can be NULL which indicates that any UAV access could require the barrier
 ...
 */
 ...
 // Either the beforeResource or the afterResource parameters can be NULL which indicates that any placed or reserved resource could cause aliasing.
	...
	// Returns the number of resource barriers that were flushed to the command list
...
private:
	// An vector of resource barriers
	using ResourceBarriers = std::vector<D3D12_RESOURCE_BARRIER>;

	// Pending resource transitions are committed before a command list is executed on the command queue. This guarantees that resources will be in the expected state at the beginning of a command list
	ResourceBarriers pendingResourceBarriers;

	// Resource barriers that need to be committed to the command list (non-pending RBs)
	...
	struct ResourceState
	{
		...
		// If the specified subresource is not found in the SubresourceState array (map) then the state of the resource (D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES) is returned
	...
	inline static ResourceStateMap globalResourceState{};
